...
trait HasCollectVTable<'a> {
    const VTABLE: CollectVTable;
}

impl<'gc, 'a, T, M> HasCollectVTable<'a> for (PhantomData<T>, M)
where
    T: ?Sized + 'a,
    M: MetaCollect<'gc, 'a, T>,
{
...
impl GcBoxHeader {
    pub const fn new<'gc, 'a, T, M>() -> Self
    where
        T: ?Sized + 'a,
        M: MetaCollect<'gc, 'a, T>,
    {
    ...
    pub unsafe fn reset_vtable<'gc, 'a, T, M>(&self)
    where
        T: ?Sized + 'a,
        M: MetaCollect<'gc, 'a, T>,
    {
